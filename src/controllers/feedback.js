...
exports.putData = (req, res, next)=>{
    ...
    feedback.updateOne({_id: _id, "data.id": id}, updateDocumentImage)
    .then(result=>{
        feedback.updateOne({_id: _id, "data.id": id}, updateDocumentName)
        .then(result=>{
            feedback.updateOne({_id: _id, "data.id": id}, updateDocumentComment)
            .then(result=>{
                res.status(201).json({
