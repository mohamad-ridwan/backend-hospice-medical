...
exports.postData = (req, res, next) => {
  ...
  doctors
    .updateOne({ id: id }, { $push: { data: data } }, { upsert: true })
    .then((result) => {
      res.status(201).json({
...
exports.putProfileDoctor = (req, res, next) => {
  ...
  function pushToUpdateData(updateDocument, options, message) {
    doctors
...
exports.putMedsos = (req, res, next) => {
  ...
  const options = {
    arrayFilters: [{ "outer.id": id }, { "inner.id": idMedsos }],
  };

  function update(document, filters) {
    doctors
      .updateOne({ _id: _id }, document, filters)
      .then((result) => {
        res.status(201).json({
          message: `${property} profile doctors berhasil di update`,
          data: result,
