...
exports.postDataBio = (req, res, next) => {
  ...
  aboutMyself
    .updateOne({ _id: _id }, { $push: { dataBio: data } }, { upsert: true })
    .then((result) => {
      res.status(201).json({
...
exports.putDataBio = (req, res, next) => {
  ...
  aboutMyself
    .updateOne({ _id: _id, "dataBio.id": id }, updateDocumentNameIcon)
    .then((result) => {
      aboutMyself
        .updateOne({ _id: _id, "dataBio.id": id }, updateDocumentTitle)
        .then((result) => {
          aboutMyself
            .updateOne({ _id: _id, "dataBio.id": id }, updateDocumentDeskripsi)
            .then((result) => {
              res.status(201).json({
